#!/usr/bin/env bash
# CPO: founder manual guide for partial E2E (signup → deploy, no trades).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
export SITE="${SITE:-https://zengtrade.in}"

echo "== Partial E2E guide (CPO) — $SITE =="
echo ""

if ! ./scripts/verify-activation-path.sh --partial; then
  echo ""
  echo "Fix probes above before manual test."
  exit 1
fi

echo ""
echo "== Manual steps (incognito) =="
echo ""
echo "1. Signup"
echo "   → $SITE/login?mode=signup"
echo "   Expect: login foot mentions Algo Studio · land on $SITE/dashboard after Google/email auth"
echo "   Pro intent: $SITE/login?mode=signup&plan=pro (Google OAuth preserves plan + UTM → checkout)"
echo "   Optional organic path: $SITE/login → Browse coin strategies (signup_coins UTM)"
echo "   Expect: land on $SITE/dashboard after Google/email auth"
echo ""
echo "2. Deploy paper strategy"
echo "   → $SITE/dashboard → Algo Studio → Library → Deploy (e.g. Trend Follower)"
echo "   Alt: visit $SITE/app → onboarding opens Algo Studio · or /app#pricing deploy-first banner"
echo "   Expect: deploy success toast + post-deploy hint → View evidence (/app#forward)"
echo "   While worker down: yellow banner on /dashboard also links View evidence → /app#forward"
echo "   Expect: no upgrade block on first deploy"
echo ""
echo "3. Trades (blocked until worker)"
echo "   → Requires fresh worker heartbeat: $SITE/ops/worker"
echo "   After P0: $SITE/app#forward within ~5–15 min"
echo ""
echo "Full checklist: $SITE/ops/e2e"
echo "Recovery: docs/WORKER_RECOVERY.md"
echo ""
echo "Growth objective:"
./scripts/print-growth-goal-summary-fast.sh 2>/dev/null | sed 's/^/  /' || true
