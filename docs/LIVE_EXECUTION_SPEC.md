# Live-Execution Spec (Crypto / Binance)
...
**Prime directive (unchanged):** *Be very safe, then make profits.*
Building this layer is not permission to trade real money. Real orders remain gated on evidence.
...
**Non-goal: sub-millisecond latency.** Impossible retail (home link to exchange is 20-250 ms of
...
1. **Cleared the honest go-live gate**, positive expectancy **net of real costs** (including
   the ~1% crypto cost model), over **≥ 50 closed trades**, **regime-fit = "fit"** in the live
   regime. Per-strategy, not book-wide.
...
5. **Starts tiny**, minimal size; scale only after live fills track paper.
...
OrderRouter.on_fill(cb)                                  # driven by the user-data stream
...
| **Perps** (Binance, roadmap) | marketable-LIMIT, IOC | **reduce-only** MARKET | reduce-only can't accidentally flip/increase |
| **Options** (roadmap) | **LIMIT within the spread, ONLY** | LIMIT, or let defined-risk expire | market on a wide option book is self-harm |
...
| Rail | Mechanism |
|---|---|
| `ALLOW_LIVE` | Two-key OS-env lock. Absent/one-key → paper. |
...
- **Phase 1, One strategy live.** Arm the *single* gate-cleared strategy, smallest size, one symbol,
  `ALLOW_LIVE` + allowlist. Watch **live fills vs paper** to measure real slippage vs the modelled edge.
- **Phase 2, Widen slowly.** Only if live tracks paper within tolerance for N sessions, add the next
  gate-cleared strategy. Repeat.
- **Never** arm the whole book at once.
...
**Integration (shadow, live feeds, full session):**
...
- [ ] Shadow clean for **N sessions** (define N, e.g. 5).
...
1. `BinanceBroker`; `OrderRouter` with idempotency + reconciliation. **Shadow.**
2. Websocket feed adapters (Binance streams) + staleness guard + reconnect.
...
**Everything in `saas/worker/bot/` (signals, edge-gate, profit-lock, runner, governor, regime-fit,
cull) is reused unchanged.** Live changes only HOW an intent becomes a fill, never WHAT the intent is.
